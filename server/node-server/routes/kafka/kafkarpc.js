...
KafkaRPC.prototype.makeRequest = function(topicName, content, callback){
    ...
    self.setupResponseQueue(self.producer,topicName,function(){
        console.log('in response');
        //put the request on a topic

        let payloads = [
            { topic: topicName, messages: JSON.stringify({
                correlationId:correlationId,
                replyTo:'responseTopic',
                data:content}),
                partition:0}
        ];
        console.log('in response1', payloads[0]);
...
KafkaRPC.prototype.setupResponseQueue = function(producer,topicName, next){
    ...
    let consumer = self.connection.getConsumer('responseTopic');
